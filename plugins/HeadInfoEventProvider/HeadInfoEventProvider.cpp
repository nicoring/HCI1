#include "HeadInfoEventProvider.h"
...
MTQ_REGISTER_PLUGIN(HeadInfoEventProvider)
...
{
	HeadInfoEventProvider* this_ = (HeadInfoEventProvider*) pUserData;
	//qDebug() << data->iFrame;
...
HeadInfoEventProvider::HeadInfoEventProvider(const QString &pluginName, const Configuration &config)
	: Plugin(pluginName, config)
{
}
/*
void HeadInfoEventProvider::updateHead(sRigidBodyData *data) {

	Vector3 pos(data->x,data->y,data->z);
	Matrix4x4 direction;
	direction.rotate(Quaternion(data->qw,data->qx, data->qy, data->qz));
	Vector3 directionVector = Vector3(0,1,0)*direction;

	if (directionVector.z()>0) return;
	Vector3 viewPos = pos+directionVector*abs(pos.z()/directionVector.z());
...
void HeadInfoEventProvider::updateHead(sRigidBodyData *data) {
	...
	Matrix4x4 direction;
	direction.rotate(Quaternion(data->qw,data->qx, data->qy, data->qz));
	Vector3 directionVector = Vector3(0, 0, 1)*direction;
	if (directionVector.y()>0) return;
	Vector3 viewPos = pos+directionVector*abs(pos.y()/directionVector.y());
	MTQ_DEBUG << "emiting headpointevent";
	MTQ_EMIT(new HeadPointingEvent(Vector2(viewPos.z(), -viewPos.x()), mtq::User()));
	qreal rotationOnFloor = std::atan2(-directionVector.x(), directionVector.z());
	MTQ_EMIT(new HeadEvent(Vector2(pos.z(), -pos.x()), rotationOnFloor, mtq::User()))
}

Vector3 HeadInfoEventProvider::transformToWorld(Vector3 vec) {
	return vec;
}

void HeadInfoEventProvider::initialize()
{
}

void HeadInfoEventProvider::run()
{
...
void HeadInfoEventProvider::serverLookupCallback(QHostInfo server)
{
//TODO: check and call stage2
}


void HeadInfoEventProvider::run_stage2() {
