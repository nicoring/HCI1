...
uniform sampler2D qt_Texture3;
uniform sampler2D qt_Texture4;
...
void main(void)
{
    ...
    if (light0BrightnessMultiplicator > 0.0){
      for (int i = 0; i <4; i++){
         if (texture2D(qt_Texture1, light0shadowCoord.xy + poissonDisk[i]/500).z < (light0shadowCoord.z - bias)){
             l0shadowMult -= 0.20;
         }
      }      
    }
    ...
    if (light1BrightnessMultiplicator > 0.0){
      for (int i = 0; i <4; i++){
         if (texture2D(qt_Texture2, light1shadowCoord.xy + poissonDisk[i]/500).z < (light1shadowCoord.z - bias)){
             l1shadowMult -= 0.20;
         }
      }      
    }
    ...
    if (light2BrightnessMultiplicator > 0.0){
      for (int i = 0; i <4; i++){
         if (texture2D(qt_Texture3, light2shadowCoord.xy + poissonDisk[i]/500).z < (light2shadowCoord.z - bias)){
             l2shadowMult -= 0.20;
         }
      }      
    }
    ...
    if (light3BrightnessMultiplicator > 0.0){
      for (int i = 0; i <4; i++){
         if (texture2D(qt_Texture4, light3shadowCoord.xy + poissonDisk[i]/500).z < (light3shadowCoord.z - bias)){
             l3shadowMult -= 0.20;
         }
      }      
    }  
    ...
    vec3 lightsFinal = light0final + light1final;
